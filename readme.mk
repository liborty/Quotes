...
The solution adopted here by Quotes is to use *markdown* for efficient human input. Additionally, the fact that it is just plain text lends it to easy machine processing by standard *Unix/Linux* utilities such as *sed* and *pandoc*. Thus the straigth 'computer quotes' are speed typed into the markdown file and Quotes then converts this into .doc .odt or similar formats demanded by publishers, with the quotation marks already properly typeset. 
...
- reads any plain text (or markdown) from input file. The filespec should end with a standard format extension understood by *pandoc*, eg. */some/path/mynovel.mk*
